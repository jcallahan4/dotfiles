...
| `MEETINGS.md` | You only | A link to `~/notes/meetings.md`, one file for all meetings across projects. Run `mtg` when a meeting starts. Start a line with the project name when a meeting covers several. The agent reads it when you refer to a meeting. |
