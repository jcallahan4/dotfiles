...
| `mtg <person> ["label"]` | Adds a dated header to `~/notes/meetings/<person>.md` and opens it in insert mode in a new Neovim tab whose working directory is `~/notes/meetings`, so `Space f f` and `Space e e` show the notes there. `Space h` closes that tab and returns to the project. `mtg` alone lists people and their latest entry. Inside a project it also creates the gitignored `meetings/` link. |
