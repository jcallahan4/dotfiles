...
| `mtg ["label"]` | Adds a dated header to `~/notes/meetings.md`, the one meeting file for all projects, and opens Neovim there in insert mode. Inside a project it also creates the gitignored `MEETINGS.md` link if missing. |
| `mtg done` | After a meeting, inside a project: commits `~/notes`, has Codex copy the newest entry's lines about this project into `docs/meetings.md` with action items, commits that, and prints decisions, what you owe, what they owe, and next steps. |
